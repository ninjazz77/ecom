...
export const verifyEmail = async (token, email, frontendBaseUrl) => {
  try {
    ...
    try {
      ...
      throw new Error(`Failed to create email transporter: ${transportError.message}`);
    }
    ...
    try {
      ...
      if (sendError.code === "EAUTH") {
        throw new Error("SMTP Authentication failed. Please check SMTP_USER and SMTP_PASS are correct.");
      } else if (sendError.code === "ESOCKET") {
        throw new Error("Cannot connect to SMTP server. Check your internet connection and SMTP_HOST/SMTP_PORT.");
      } else if (sendError.code === "ETIMEDOUT") {
        throw new Error("SMTP connection timed out. The email server may be unreachable.");
      } else {
